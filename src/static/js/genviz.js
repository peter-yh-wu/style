...
document.addEventListener("DOMContentLoaded", function(e) {
  ...
  var content_map = JSON.parse(getMeta("content_map"));
  ...
  function buildGraph(inlinks, innodes) {
    ...
    node.append("circle").attr("fill", function(d) {
      ...
      document.getElementById("text").innerHTML = d.text;
  ...
  datanodes.push({"name":content_map[0][data[0]["source"]], color:maincolor, text:content_map[1][data[0]["source"]]});
  for (var k, k = 1; k < data.length; k++) {
    ...
    if (k > topn) {
      ...
      datanodes.push({"name":content_map[0][data[k]["target"]], color:"#DB4437", page:pages.indexOf(data[k]["target"]) > -1, text:content_map[1][data[k]["target"]]});
      order[key] = k;
    }
